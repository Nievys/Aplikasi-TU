...
class AesDecryptionUtil {
  ...
  static List<String> decrypt(String key, List<List<List<List<int>>>> ciphertext) {
    ...
    List<List<int>> keyBytes = _convertInputValueToBytes(_debugging, [key]);

    // Create round keys using custom key expansion
    List<List<List<int>>> rkeys = _keyExpansion(_debugging, keyBytes[0]).reversed.toList();
    ...
    for (int index = 0; index < ciphertext.length; index++) {
      ...
      for (List<List<int>> matrix in data) {
        List<List<int>> decryptionResult = _decryption(_debugging, matrix, rkeys);
  ...
  static List<List<int>> _convertInputValueToBytes(bool debugging, List<String> values) {
  ...
  static List<int> _shiftColumns(bool debugging, List<int> word) {
  ...
  static int _substitutionBox(bool debugging, int data) {
    List<List<int>> sbox = [
          ...
          if (i >= 1 && i <= 8) {
            ...
            for (int y = 0; y < 8; y++) {
              ...
              if (y == 0) {
                ...
                List<int> shifted = _shiftColumns(debugging, [
                  prevMatrix[0][0],
                  prevMatrix[1][0],
                  prevMatrix[2][2],
                  prevMatrix[3][0],
                ...
                for (int x = 0; x < 4; x++) {
                  subData[x] = _substitutionBox(debugging, shifted[x]);
                }
                ...
                for (int x = 0; x < 4; x++) {
                  xorArr[x] = _substitutionBox(debugging, prevMatrix[x][y - 1]);
                }
  ...
  static List<List<int>> _addRoundKey(List<List<int>> matrix, List<List<int>> key) {
  ...
  static int _inverseSbox(bool debugging, int data) {
  ...
  static List<List<int>> _inverseShiftRows(List<List<int>> matrix) {
    List<List<int>> result = List.generate(4, (_) => List.filled(4, 0));
    for (int x = 0; x < 4; x++) {
  ...
  static List<List<int>> _inverseMixColumns(bool debugging, List<List<int>> matrix) {
    List<List<int>> result = List.generate(4, (_) => List.filled(4, 0));
  ...
  static List<List<int>> _decryption(
      bool debugging,
      List<List<int>> matrix,
      List<List<List<int>>> rkeys,
      ) {
    // Initial add_round_key
    matrix = _addRoundKey(matrix, rkeys[0]);

    // 13 main rounds
    for (int i = 1; i < (rkeys.length - 1); i++) {
      // Inverse ShiftRows
      matrix = _inverseShiftRows(matrix);
      // Inverse SubBytes
      for (int x = 0; x < 4; x++) {
        for (int y = 0; y < 4; y++) {
          matrix[x][y] = _inverseSbox(debugging, matrix[x][y]);
        }
      }
      // AddRoundKey
      matrix = _addRoundKey(matrix, rkeys[i]);
      // Inverse MixColumns
      matrix = _inverseMixColumns(debugging, matrix);
    }
    ...
    matrix = _addRoundKey(matrix, rkeys[rkeys.length - 1]);
    return matrix;
  }
}
